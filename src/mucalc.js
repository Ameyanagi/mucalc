...
// make regular expression to extract chemical formula
// e.g. H2O -> {H: 2, O: 1}
// e.g. H2O2 -> {H: 2, O: 2}
// e.g. CuO -> {Cu: 1, O: 1}
...
const getZ = (formula) => {
    ...
    for (let i = 0; i < atoms.length; i++) {
        ...
        let name = atom.match(/[A-Z][a-z]?/g);
        z[name] = num;
    }
    return z;
}

const mcmaster = (ephot, fit) => {
    let i;
    let xsec = 0.0;
    let log_e = Math.log(ephot);
...
const mucal = (atom_name, ephot) => {
    ...
    if (!(element.includes(atom_name))) {
        console.log(`MuCal: ${atom_name} is not a valid element.`);
        return -1;
    }
    ...
    if (ephot < 0.0) {
        console.log(`MuCal: photonelectron energy must be non-negative.`);
        return -1;
    }
    ...
    let xsec = {
        ...
        "cm2_ncoh": 0.0,
        "cm2g_tot": 0.0,
        "conversion": conv_fac[Z],
        "absorption": 0.0,
        "atomic_weight": at_weight[Z],
        "density": density[Z],
        "l1_jump": 0.0,
        "l2_jump": 0.0,
        "l3_jump": l3_jump[Z],
    ...
    let fluo = {
        "k_yield": k_yield[Z],
        "l1_yield": l_yield[Z][0],
        "l2_yield": l_yield[Z][1],
        "l3_yield": l_yield[Z][2],
    }
    ...
    if (Math.abs(xsec.k_edge - ephot) < 0.001 ||
        Math.abs(xsec.l1_edge - ephot) < 0.001 ||
        Math.abs(xsec.l2_edge - ephot) < 0.001 ||
        Math.abs(xsec.l3_edge - ephot) < 0.001 ||
        Math.abs(xsec.m_edge - ephot) < 0.001) {
        console.log(`MuCal: Photon energy is within 1 eV of edge fit results may be inaccurate.`);
    }
    ...
    switch (shell) {
        case 1:
            xsec.barn_photo = mcmaster(ephot, k_fit[Z]);
            break;
        ...
        case 3:
            xsec.barn_photo = mcmaster(ephot, m_fit[Z]);
            break;

        case 4:
            xsec.barn_photo = mcmaster(ephot, n_fit[Z]);
            break;
    ...
    xsec.barn_coh = mcmaster(ephot, xsect_coh[Z]);
